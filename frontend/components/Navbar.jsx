...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <nav className="mx-auto grid min-h-18 max-w-7xl grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 sm:px-6 md:flex md:flex-nowrap md:py-0 lg:px-8" aria-label="Main navigation">
        <Link className="col-start-1 row-start-1 flex min-w-0 items-center gap-3" href={user ? "/dashboard" : "/"} aria-label="Blog Application home">
        ...
        <form action="/#latest-blogs" className="relative col-span-full col-start-1 row-start-2 w-full min-w-0 md:order-none md:mx-auto md:max-w-md" method="get" role="search">
          ...
          <input className="w-full rounded-xl border border-slate-300 bg-slate-50 py-2.5 pl-11 pr-4 text-sm outline-none transition placeholder:text-slate-400 focus:border-violet-400 focus:bg-white focus:ring-4 focus:ring-violet-100" id="public-navbar-search" name="title" placeholder="Search by blog title..." type="search" />
        </form>

        <div className="col-start-3 row-start-1 justify-self-end md:ml-auto">
          {user ? (
            <ProfileMenu showRole />
          ) : (
