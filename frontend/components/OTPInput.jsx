export default function OTPInput({ value, onChange, error, disabled = false }) {
  return (
    <label className="block text-sm font-semibold text-slate-700">
      ...
      <input aria-describedby={error ? "otp-error" : "otp-help"} aria-invalid={Boolean(error)} autoComplete="one-time-code" className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-center text-xl font-bold tracking-[0.5em] text-slate-900 outline-none transition placeholder:text-slate-300 focus:border-violet-500 focus:ring-4 focus:ring-violet-100 disabled:bg-slate-100" disabled={disabled} inputMode="numeric" name="otp" onChange={onChange} pattern="[0-9]{6}" placeholder="000000" required value={value} />
