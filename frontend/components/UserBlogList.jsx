...
export default function UserBlogList({ title = "", category = "", notice = "" }) {
  ...
  useEffect(() => {
    ...
    async function loadBlogs() {
      ...
      setError("");
      try {
        const response = await blogService.getAll({ title, category });
        const allBlogs = Array.isArray(response?.data) ? response.data : [];
        const visibleBlogs = user.role === "admin" ? allBlogs : allBlogs.filter((blog) => Number(blog.userId) === Number(user.id));
        if (isActive) setBlogs(visibleBlogs);
      } catch (requestError) {
  ...
  }, [category, title, user.id, user.role]);
  ...
  if (isLoading) {
    return <div className="mt-8 grid animate-pulse gap-4 sm:grid-cols-2"><div className="h-40 rounded-2xl bg-slate-200" /><div className="h-40 rounded-2xl bg-slate-200" /><span className="sr-only">Loading blogs...</span></div>;
  }

  return (
    <div className="mt-8">
      {success ? <div className="mb-5 flex items-center justify-between gap-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800" role="status"><span>{success}</span><button aria-label="Dismiss message" className="font-bold" onClick={() => setSuccess("")} type="button">×</button></div> : null}
      ...
      {blogs.length === 0 ? (
        ...
        <p className="font-bold text-slate-800">{title || category ? "No blogs found." : user.role === "admin" ? "No blogs found." : "You have not created any blogs yet."}</p>
        <p className="mt-2 text-sm text-slate-500">{title || category ? "Try another title or category." : user.role === "admin" ? "No blog has been published yet." : "Create your first blog to get started."}</p>
        {!title && !category ? <Link className="mt-5 inline-block rounded-xl bg-violet-600 px-5 py-3 text-sm font-bold text-white" href="/dashboard/blogs/create">Create Blog</Link> : null}
        </div>
      ) : (
        <>
